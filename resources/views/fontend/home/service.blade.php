<?php 
    $slide_sList = App\Model\M_slide_img::where('slide_id',$row->slide_id)->get();
?>
 <section class="newservices-section">
    <div class="container">
        <h2 class="webgtm__title text-center  ">{{$row->title}}</h2>
        <div class="webgtm__descr text-center  ">{{$row->title_2}}</div>
        
        <div class="newservices-section__list">
            @foreach($slide_sList as $key => $slide_s)
                    <a href="{{$slide_s->link}}" class="newservices-section__item  ">
